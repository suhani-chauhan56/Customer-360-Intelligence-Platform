...
SELECT c.city, c.state, COUNT(DISTINCT o.customer_id) AS customers, ROUND(SUM(o.revenue), 2) AS revenue
FROM
    fact_orders AS o
    JOIN dim_customer AS c ON c.customer_id = o.customer_id
WHERE
...
WITH
    category_customer_orders AS (
        SELECT COALESCE(
                p.category_name_english, 'unknown'
            ) AS category, o.customer_id, COUNT(DISTINCT o.order_id) AS order_count
        FROM
            fact_orders AS o
...
SELECT
    ...
    ROUND(
        100.0 * SUM(order_count > 1) / NULLIF(COUNT(*), 0),
        2
    ) AS repeat_rate_pct
...
SELECT
    ...
    ROUND(
        100.0 * SUM(f.opened) / NULLIF(SUM(f.email_sent), 0),
        2
    ) AS open_rate_pct,
    ROUND(
        100.0 * SUM(f.clicked) / NULLIF(SUM(f.email_sent), 0),
        2
    ) AS ctr_pct,
    ROUND(
        100.0 * SUM(f.converted) / NULLIF(SUM(f.clicked), 0),
        2
    ) AS click_conversion_pct,
    ROUND(SUM(f.revenue_generated), 2) AS revenue,
    ROUND(d.campaign_cost, 2) AS cost,
    ROUND(
        (
            SUM(f.revenue_generated) - d.campaign_cost
        ) / NULLIF(d.campaign_cost, 0),
        2
    ) AS roi
FROM
    fact_campaign AS f
